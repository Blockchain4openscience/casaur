...
/**
 * Record a list of granted degrees
 ...
 * @param  {org.degree.PersonalizeCertificate} transaction - the transaction parameters
 * @transaction
 */
async function PersonalizeCertificate(transaction) {
  const personalCertificateRegistry = await getAssetRegistry('org.degree.PersonalCertificate');
  const factory = getFactory();
  const serializer = getSerializer();
  ...
  const certificate = factory.newResource('org.degree', 'PersonalCertificate', recipientsInfo.certId);
  certificate.templateId = transaction.templateId;
  certificate.administrator = transaction.administrator;
  certificate.recipient = recipientsInfo.recipient;
  certificate.recipientProfile = recipientsInfo.recipientProfile;
  certificate.hash = sha256(JSON.stringify(serializer.toJSON(certificate, {
      validate: false, 
      convertResourcesToRelationships: true,
      permitResourcesForRelationships: false,
